...
- 首次进入切割仍要求 <code>Pointing_Up</code> 且姿势置信度至少 <code>0.68</code>。2026-09-15 经用户确认增加连续跟随容错：此后最多 150ms 的低分食指或未知姿势，可在 21 个坐标完整有效、食指仍伸展且指尖无异常跳变时沿当前观测位置继续；弱识别不能延长期限，超时后必须重新确认。具体伸展保护以 <code>DESIGN.md</code> 的活动输入说明为准。
- 张掌、握拳及其他明确姿势立即断开，未知姿势下食指弯曲也断开；无手时立即断开且不跨缺失区间补刀。姿势分类分数与手部坐标是否存在分开判断。
- 连续丢失手部超过 600ms 时自动暂停计时，并显示“手暂时离开画面”；恢复后继续。
- 同一物体命中去重窗口 120ms。
- 2026-09-15 快划调整：游戏使用实际采样间隔和手掌、指尖的共同移动检查大位移，保留 0.6 归一化距离硬上限及真实丢手断线；已接受轨迹不因平滑追赶被重复截断。详细保护参数集中记录在 <code>DESIGN.md</code>。
- 手势正式局运动速度为原来的 78%，重力按速度比例的平方调整，保留抛物线高度与横向落点；普通阶段抛出间隔从 1150ms 改为 1500ms。重玩保留输入模式；普通输入、教学、减少动态轨迹及 45 秒阶段计时沿用原规则。
- 视觉目标直径建议 88–120 CSS px，手势碰撞半径在视觉半径上扩大 20%。
- 坐标继续镜像，保证屏幕上的手和游标移动方向一致。
- 游戏入口开启 1.8 倍位移增益，将镜头中央约 55.6% 的指尖活动范围映射到整个舞台，帮助用户减少向镜头边缘伸手。先在原始坐标中检查跳变、平滑和小幅抖动，再统一映射刀锋显示与命中坐标；边界夹取和重新建锚规则见 <code>DESIGN.md</code>。
- 快划跟随按采集时间和实际移动速度调整平滑，慢移保留去抖。重新确认手指的首帧可只定位刀锋，不跨丢失区间补刀；手势命中仍即时计算，绘制合并到显示帧。参数和 `cursor` / `resume` 事件约定集中记录在 <code>DESIGN.md</code>。
...
- 首帧只建立锚点，不生成超长线段。
- 两个有效帧生成一个归一化线段。
- 未确认食指前，置信度低于 0.68 或未知姿势不输出。
- 确认后低分食指 / 未知姿势按 150ms 有限容错继续，超时、坐标缺失或食指弯曲即断开。
- 张掌、握拳等其他明确姿势不生成剥壳轨迹。
- 坐标跳变超阈值时重置锚点。
- 同目标 120ms 内不重复命中。
...
- 验证丢手 600ms 暂停、恢复继续。
- 验证低置信度或未知姿势不能启动切割；确认食指后仅有限容错。Open Palm、Closed Fist、食指弯曲和坐标缺失立即断开，恢复时不跨断点误切。
